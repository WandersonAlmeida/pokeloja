...
function render(pokemonsList) {
  ...
  const pokemonsObjectList = pokemonsList.map(
    (pokemon) => new Pokemon(pokemon.name, pokemon.url)
  );
  ...
  btnAddPokemon.forEach((btn) => {
    btn.addEventListener("click", (event) => {
      ...
      const pokemon = pokemonsObjectList.find(
        (pokemonObject) => pokemonObject.id == id
      );
      addPokemon(pokemon);
    });
  });
...
async function getPokemons(page = 0) {
  const listaPokemon = document.querySelector(".listaPokemon");
  listaPokemon.innerHTML =
    '<div class = "carregando">Carregando pokemons...</dic>';
  await fakePromise();
  const limit = 20;
  const response = await fetch(
    `https://pokeapi.co/api/v2/pokemon?limit=${limit}&offset= ${limit * page}`
  );
